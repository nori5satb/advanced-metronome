...
export default function MetronomeDisplay({ className = '' }: MetronomeDisplayProps) {
  const { currentBeat, settings, isPlaying } = useMetronome();
      ...
      {isPlaying && currentBeat && (
        <div className="text-center">
          <div className="text-lg font-medium text-gray-700">
            Measure {currentBeat.measure}, Beat {currentBeat.beat}
          </div>
          {currentBeat.isDownbeat && (
            <div className="text-sm text-red-600 font-semibold">Downbeat</div>
          )}
        </div>
      )}
